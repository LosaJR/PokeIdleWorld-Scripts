...
(() => {
  'use strict';
  if (window.__pgHuntIntelligenceCoreV112) return;
  window.__pgHuntIntelligenceCoreV112 = true;
...
(() => {
  'use strict';
  if (window.__pgHuntIntelligenceItemCoreV112) return;
  window.__pgHuntIntelligenceItemCoreV112 = true;
...
(() => {
  'use strict';
  if (window.__pgHuntIntelligenceEngineV112) return;
  window.__pgHuntIntelligenceEngineV112 = true;
  ...
  console.info('[Hunt Intelligence] Motor v1.1.2 cargado: VIP manual, bonus diario, ranking personal y Pokédex unificados.');
})();


(() => {
  'use strict';
  if (window.__pgHuntIntelligenceSupervisorV112) return;
  window.__pgHuntIntelligenceSupervisorV112 = true;
  ...
  function adoptLegacyVip() {
    const vip=Boolean(window.__PGPiwToolsEngine?.getVip?.()); let changed=0;
    segments=segments.map(row=>{
      if(row.accountId!==accountId()||row.vipKnown===true)return row;
  ...
  console.info('[Hunt Intelligence] Supervisor unificado v1.1.2 cargado: VIP y bonus diario normalizados.');
})();

(() => {
  'use strict';
  if (window.__pgHuntIntelligenceUiV112) return;
  window.__pgHuntIntelligenceUiV112 = true;
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .pg-u-row{display:grid;grid-template-columns:34px minmax(175px,1fr) repeat(4,minmax(76px,auto));gap:8px;align-items:center;padding:9px 8px;border-bottom:1px solid #202b3b;font-size:11px}
      ...
      #${PANEL_ID} .pg-u-metric{text-align:right;font-variant-numeric:tabular-nums}.xp{color:#72b7ff}.gold{color:#f2cc60}.loot{color:#8ce99a}.score{color:#d4a6ff}.rate{color:#d5a6ff}.speed{color:#72b7ff}.items{color:#8ce99a}.eff{color:#ffd36c}
      ...
      @media(max-width:700px){#${PANEL_ID} .pg-u-sourcebox{grid-template-columns:1fr}.pg-u-daily{white-space:normal;flex-wrap:wrap}#${PANEL_ID} .pg-u-row{grid-template-columns:28px 1fr 82px 82px}#${PANEL_ID} .hide-mobile{display:none}#${PANEL_ID} .pg-u-settings{grid-template-columns:1fr}#${PANEL_ID} .pg-u-tabs{top:47px}#${PANEL_ID} .pg-hi-grid{grid-template-columns:1fr}#${PANEL_ID} .pg-hi-hero{grid-template-columns:1fr 1fr}#${PANEL_ID} .pg-hi-history-head{display:none}#${PANEL_ID} .pg-hi-history-row{grid-template-columns:1fr 70px 44px}#${PANEL_ID} .pg-hi-history-name{grid-column:1/-1}}
    `;
    document.head.appendChild(style);
  }
  ...
  function shell(bodyHtml, options = {}) {
    ...
    overlay.addEventListener('click', event => {
      ...
      if (event.target.closest('[data-clear-intelligence-history]')) {
        event.preventDefault(); event.stopPropagation();
        if (confirm('¿Borrar todo el histórico y las calibraciones de esta cuenta?')) S()?.clearHistory?.();
        renderHistory(false); return;
      }
  ...
  install();
  console.info('[Hunt Intelligence] v1.1.2 cargado: Hunts, No capturados, Item Finder, supervisor e histórico unificados.');
})();
