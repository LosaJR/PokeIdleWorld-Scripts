...
(() => {
  ...
  const NPC_GOLD_EXCLUDED_SPECIAL_NAMES = new Set([
    'strange pheromone', 'strange pheromones',
    'bronze boss token', 'bronze boss tokens', 'bronze token', 'bronze tokens'
  ]);

  function isNpcGoldExcludedSpecial(item) {
    return NPC_GOLD_EXCLUDED_SPECIAL_NAMES.has(norm(item?.name || item?.title || ''));
  }
  ...
  function ingestMeasurements(data) {
    ...
    for (const session of sessions) {
      ...
      for (const [id, drop] of Object.entries(session.drops || {})) {
        ...
        lootUnits += qty;
        // Oro NPC real excluye los dos drops especiales de altísimo valor.
        // Siguen midiéndose por separado en Especiales/h.
        if (!isNpcGoldExcludedSpecial(item)) {
          lootGold += qty * finite(item?.npcPrice, item?.sellValue, 0);
        }
      }
  ...
  window.__PGUnifiedHuntCore = {
    ...
    looksLikeMapButton,
    getData: force => loadData(Boolean(force)),
    getLeadPokemon: () => getLeadPokemon(),
    getCharacter: () => getCharacter(),
    huntRequiredLevel: hunt => huntRequiredLevel(hunt),
    isUnlocked: (hunt, lead) => isUnlocked(hunt, lead),
    getConfig: () => Object.assign({}, config),
    setMode: value => { config.mode = value === 'general' ? 'general' : 'xp'; saveState(); },
    setWeight: (key, value) => {
      if (!['xpWeight','lootWeight','rareWeight','specialWeight','goldWeight'].includes(key)) return;
...
(() => {
  'use strict';
  if (window.__pgHuntIntelligenceItemCoreV1138) return;
  window.__pgHuntIntelligenceItemCoreV1138 = true;
...
(() => {
  'use strict';
  if (window.__pgHuntIntelligenceEngineV1138) return;
  window.__pgHuntIntelligenceEngineV1138 = true;
  ...
  function personalIntelligence(lead, hunt, diff, dailyBoosted, vipActive, expected = {}) {
    try {
      return window.__PGHuntIntelligenceSupervisor?.getPersonalEstimate?.({
        lead,
        hunt,
        diff,
        dailyBoosted: Boolean(dailyBoosted),
        vipActive: Boolean(vipActive),
        expectedKph: finite(expected?.kph, diff?.kph),
        expectedXph: finite(expected?.xph)
      }) || null;
    } catch { return null; }
  }
  ...
  function enemyStats(creature, levelOverride = null) {
    const level = Math.max(1, finite(levelOverride, creature?.huntLevel, creature?.level, 1));
    ...
    return { level, hp: values[0], atk: values[1], def: values[2], spAtk: values[3], spDef: values[4], speed: values[5] };
  }
  ...
  function bestMoveDamage(lead, species, target, includeTM = false, targetLevel = null) {
    const level = Math.max(1, finite(lead?.level, 1));
    const pStats = playerStats(lead, species);
    const eStats = enemyStats(target, targetLevel);
  ...
  function modelHunt(hunt, lead, species, productivity, includeTM = false) {
    const creature = hunt.creature || {};
    const targetLevel = huntRequiredLevel(hunt);
    const offense = bestMoveDamage(lead, species, creature, includeTM, targetLevel);
    ...
    return {
      level: targetLevel,
      targetLevel,
      playerLevel: Math.max(1, finite(lead?.level, 1)),
      playerStats: { ...offense.pStats },
      enemyStats: { ...offense.eStats },
  ...
  const SPECIAL_ITEM_NAMES = new Set(['strange pheromone', 'strange pheromones', 'bronze boss token', 'bronze boss tokens', 'bronze token', 'bronze tokens']);

  function specialItemIds(data) {
    const ids = new Set();
    const byId = data?.itemsById;
    if (byId && typeof byId.forEach === 'function') {
      byId.forEach((item, id) => {
        if (SPECIAL_ITEM_NAMES.has(norm(item?.name || ''))) ids.add(String(item?.id ?? id));
      });
    }
    return [...ids];
  }
  ...
  function expectedLoot(creature, data, multiplier = 1) {
    ...
    const npcGoldByItemId = {};
    const specials = specialItemIds(data);
    ...
    for (const entry of entries) {
      ...
      // Especiales/h combina Strange Pheromone + Bronze Boss Token a valor 1:1.
      // Si el cliente expone un rate oficial válido, usa el mismo multiplicador de
      // loot que el resto. En cuanto hay histórico real, ese histórico tiene prioridad.
      const specialName = norm(item?.name || pick(entry, ['name', 'itemName', 'item.name'], ''));
      const isSpecial = SPECIAL_ITEM_NAMES.has(specialName);
      if (isSpecial) {
        if (chance > 0) {
          specialUnits += expected;
          specialKnown++;
        }
      }

      // Oro NPC/h excluye Pheromone y Bronze Boss Token tanto de la estimación
      // como de las muestras reales: su enorme valor NPC distorsionaría el ranking.
      // Esos drops siguen estando representados exclusivamente en Especiales/h.
      if (!isSpecial) {
        gold += expected * npc;
        const itemId = pick(entry, ['itemId', 'id', 'item.id'], item?.id ?? null);
        if (itemId !== null && itemId !== undefined && itemId !== '' && npc > 0) {
          npcGoldByItemId[String(itemId)] = Math.max(0, npc);
        }
      }
      known++;
    }
  ...
  function scoreRows(rows, cfg) {
    ...
    const maxSpecial = maximum('specialPh');
    const maxGold = maximum('netGoldPh');
    const totalWeight = Math.max(1,
      finite(cfg.xpWeight) + finite(cfg.lootWeight) + finite(cfg.rareWeight) + finite(cfg.specialWeight) + finite(cfg.goldWeight)
    );
    rows.forEach(row => {
      row.generalScore = 100 * (
        ...
        (maxSpecial ? row.specialPh / maxSpecial : 0) * finite(cfg.specialWeight) +
  ...
  async function calculatePiwRecommendations(force = false) {
    ...
    const rows = accessibleHunts.map(hunt => {
      ...
      const exact = personalIntelligence(
        lead,
        hunt,
        diff,
        boosted,
        vipActive,
        { kph: theoreticalKph, xph: theoreticalXph }
      );
      ...
      const usePersonalSpecial = Boolean(
        usePersonal
        && exact?.specialSamples > 0
        && Number.isFinite(Number(exact?.currentSpecialItemsPh))
      );
      const usePersonalGold = Boolean(
        usePersonal
        && exact?.goldSamples > 0
        && Number.isFinite(Number(exact?.currentNpcGoldPh))
      );
      ...
      const specialDataKnown = Boolean(usePersonalSpecial || loot.specialKnown > 0);
      const specialPh = usePersonalSpecial ? Number(exact.currentSpecialItemsPh) : (loot.specialKnown > 0 ? effectiveKph * loot.specialUnits : 0);
      const netGoldPh = usePersonalGold ? Number(exact.currentNpcGoldPh) : effectiveKph * loot.gold;
      ...
      sourceParts.push(
        usePersonal
          ? `ranking: últimas ${exact.samples}/${exact.windowSamples || 12} muestras · vel ×${finite(exact.speedFactor, 1).toFixed(3)} · XP ×${finite(exact.xpFactor, 1).toFixed(3)} (${exact.confidence})`
          : 'ranking: PIWTools'
      );
      ...
      sourceParts.push(usePersonalSpecial ? `especiales: histórico real (${exact.specialSamples})` : (loot.specialKnown ? 'especiales: rate visible' : 'especiales: esperando muestras'));
      sourceParts.push(usePersonalGold ? `oro NPC: histórico real sin especiales (${exact.goldSamples})` : 'oro NPC: estimación sin especiales');
      return {
        ...
        lootPh, rarePh, specialPh, specialDataKnown, capturePh: 0, captureGoldPh: 0, supplyGoldPh: 0, netGoldPh,
        lootDataKnown: loot.known, rareLootKnown: loot.rareKnown, specialLootKnown: loot.specialKnown, lootDataTotal: loot.total,
        rareItemIds: loot.rareItemIds, specialItemIds: loot.specialItemIds, npcGoldByItemId: loot.npcGoldByItemId,
        ...
        specialRankingSource: usePersonalSpecial ? 'historico-real' : (loot.specialKnown ? 'estimado' : 'sin-rate'),
        goldRankingSource: usePersonalGold ? 'historico-real-sin-especiales' : 'estimado-sin-especiales',
        rankingSource: usePersonal ? 'historico-reciente' : 'piwtools'
      };
    ...
    rows.sort((a, b) => cfg.mode === 'general'
      ? b.generalScore - a.generalScore
        || (finite(cfg.specialWeight, 0) > 0 ? Number(Boolean(b.specialDataKnown)) - Number(Boolean(a.specialDataKnown)) : 0)
        || b.xph - a.xph
      : b.xph - a.xph || b.netGoldPh - a.netGoldPh);
    ...
    return {
      ...
      specialKnownRowsCount: rows.filter(row => row.specialDataKnown).length,
  ...
  console.info('[Hunt Intelligence] Motor v1.1.39 cargado: histórico móvil de 12 muestras por Hunt + Pokémon y calibración reciente separada de kills/h y XP/h.');
})();


(() => {
  'use strict';
  if (window.__pgHuntIntelligenceSupervisorV1138) return;
  window.__pgHuntIntelligenceSupervisorV1138 = true;
  ...
  const HISTORY_BACKUP_KEY = `${NS}:segments-backup-v1`;
  const HISTORY_CLEARED_KEY = `${NS}:segments-explicitly-cleared-v1`;
  const DEFAULT_CONFIG = { threshold: 80, minMinutes: FIXED_MIN_MINUTES, minKills: 30, refreshSeconds: 15 };
  const HISTORY_WINDOW_SAMPLES = 12;
  const HISTORY_TRIM_MIN_SAMPLES = 8;
  ...
  function rollingHistoryKey(row) {
    const acct = compact(row?.accountId || 'cuenta-local') || 'cuenta-local';
    const leadKey = compact(row?.leadId || row?.leadName || 'unknown');
    const huntKey = compact(row?.huntKey || row?.slug || row?.huntName || 'unknown');
    return `${acct}|${leadKey}|${huntKey}`;
  }

  function historySampleValid(row) {
    return Boolean(
      row
      && finite(row.elapsedSeconds) === SAMPLE_WINDOW_SECONDS
      && row.calibrationValid === true
      && rowExpectedKphForCalibration(row) > 0
    );
  }

  function trimRollingHistory(source = segments) {
    const groups = new Map();
    const ordered = (Array.isArray(source) ? source : [])
      .filter(row => row && typeof row === 'object')
      .sort((a, b) => finite(a.end, a.start) - finite(b.end, b.start));

    for (const row of ordered) {
      if (!historySampleValid(row)) continue;
      const key = rollingHistoryKey(row);
      const bucket = groups.get(key) || [];
      bucket.push(row);
      if (bucket.length > HISTORY_WINDOW_SAMPLES) bucket.splice(0, bucket.length - HISTORY_WINDOW_SAMPLES);
  ...
  function trimmedFactor(values, fallback = 1) {
    ...
    if (!sorted.length) {
      return { value: fallback, total: 0, used: 0, discarded: 0 };
    }

    const trimmed = sorted.length >= HISTORY_TRIM_MIN_SAMPLES
      ? sorted.slice(1, -1)
      : sorted;

    return {
      value: trimmed.reduce((sum, value) => sum + value, 0) / trimmed.length,
      total: sorted.length,
      used: trimmed.length,
      discarded: sorted.length - trimmed.length
    };
  }

  function recoverHistoryIfNeeded() {
    try {
      if (segments.length) return { recovered: 0, source: 'actual' };
      if (localStorage.getItem(HISTORY_CLEARED_KEY) === '1') return { recovered: 0, source: 'explicit-clear' };

      const backup = loadJson(HISTORY_BACKUP_KEY, []);
      if (Array.isArray(backup) && backup.length) {
        segments = backup.slice(-800);
        saveJson(SEGMENTS_KEY, segments);
        return { recovered: segments.length, source: 'backup' };
      }
      ...
      if (Array.isArray(legacy) && legacy.length) {
        segments = legacy.map(row => ({
          ...
          vipKnown: row.vipKnown === true,
          vipActive: row.vipKnown === true ? Boolean(row.vipActive) : null,
          cleanBaseXph: Number.isFinite(Number(row.cleanBaseXph)) ? Number(row.cleanBaseXph) : null,
          legacyBaseXph: Math.max(0, finite(row.legacyBaseXph, row.baseXph)),
          importedLegacy: true
        })).slice(-800);
        saveJson(SEGMENTS_KEY, segments);
        saveJson(HISTORY_BACKUP_KEY, segments);
        return { recovered: segments.length, source: 'legacy' };
      }
    } catch {}
  ...
  function persist() {
    config.minMinutes = FIXED_MIN_MINUTES;
    let fixedWindowsReady = false;
    try { fixedWindowsReady = localStorage.getItem(WINDOW_MIGRATION_KEY) === '1'; } catch {}
    segments = fixedWindowsReady ? trimRollingHistory(segments) : segments.slice(-800);
    rebuildCalibrationRegistry();
    saveJson(SEGMENTS_KEY, segments);
    if (segments.length) {
      saveJson(HISTORY_BACKUP_KEY, segments);
      try { localStorage.removeItem(HISTORY_CLEARED_KEY); } catch {}
    }
  ...
  function addCalibrationRow(row, registry = calibrationRegistry) {
    const expectedKph = Math.max(0, finite(row?.expectedKphAverage, row?.expectedKph));
    if (!row?.calibrationValid || !(expectedKph > 0) || !Number.isFinite(Number(row.kph))) return false;
    ...
    const factor = clamp(finite(row.kph) / expectedKph, 0.60, 1.60);
  ...
  function migrateToFixedWindows() {
    ...
    segments = trimRollingHistory(converted);
    rebuildCalibrationRegistry();
    try { localStorage.setItem(WINDOW_MIGRATION_KEY, '1'); } catch {}
    persist();
  }

  const LOOT_ZERO_FIX_KEY = `${NS}:loot-zero-parser-fix-v1`;

  function migrateBrokenLootRows() {
    try {
      if (localStorage.getItem(LOOT_ZERO_FIX_KEY)) return 0;
      let changed = 0;
      ...
      for (const row of segments) {
        if (
          ...
          && finite(row.kills) > 0
          && finite(row.items) === 0
          && finite(row.rareItems) === 0
          && finite(row.itemsPh) === 0
          && finite(row.rareItemsPh) === 0
        ) {
          row.lootMeasured = false;
          row.items = null;
          row.itemsPh = null;
          row.rareItems = null;
          row.rareItemsPh = null;
          row.cleanBaseItemsPh = null;
          row.cleanBaseRareItemsPh = null;
          row.lootInvalidatedReason = 'parser-session-drops-1.1.20';
          changed++;
        }
      }

      if (changed) saveJson(SEGMENTS_KEY, segments);
      localStorage.setItem(LOOT_ZERO_FIX_KEY, String(Date.now()));
      return changed;
    } catch {
      return 0;
    }
  ...
  function snapshotSessionDrops(session = window.__poke?.sess) {
    ...
    for (const [id, rawDrop] of Object.entries(source)) {
      const qty = Math.max(
        0,
        typeof rawDrop === 'number'
          ? finite(rawDrop)
          : finite(rawDrop?.qty, rawDrop?.quantity, rawDrop?.count)
      );
  ...
  function dropDelta(start, end, rareItemIds = [], specialItemIds = [], npcGoldByItemId = {}) {
    const from = start && typeof start === 'object' ? start : {};
    const to = end && typeof end === 'object' ? end : {};
    const rare = new Set((Array.isArray(rareItemIds) ? rareItemIds : []).map(String));
    const special = new Set((Array.isArray(specialItemIds) ? specialItemIds : []).map(String));
    const npcPrices = npcGoldByItemId && typeof npcGoldByItemId === 'object' ? npcGoldByItemId : {};
    let items = 0;
    let rareItems = 0;
    let specialItems = 0;
    let npcGold = 0;
    ...
    for (const id of ids) {
      ...
      if (special.has(String(id))) specialItems += delta;
      // Pheromone/Token se excluyen siempre del oro aunque por error exista precio.
      if (!special.has(String(id))) npcGold += delta * Math.max(0, finite(npcPrices[String(id)]));
    }
    return { items, rareItems, specialItems, npcGold };
  }
  ...
  function sampleSignature(result, current) {
    ...
    // La muestra NO se reinicia al subir de nivel o cambiar stats.
    // Eso permite medir 30 min reales mientras se levea desde nivel bajo.
    return [
      accountId(),
      compact(currentSlug()),
      compact(lead.speciesId || lead.pokeId || lead.name || ''),
      current?.dailyBoosted?'daily':'normal',
      compact(dailyTypes),
      result?.useTM?'allowtm':'notm',
      result?.vipActive?'vip':'novip'
    ].join('|');
  }

  function beginSample(raw, result, current, now = Date.now()) {
    const lead = result?.lead || {}, offense = current?.diff?.offense || {};
    const stats = current?.diff?.playerStats || {};
    activeSample = {
      ...
      baseLeadLevel: finite(lead.level),
      lastLeadLevel: finite(lead.level),
      baseAtk: finite(stats.atk),
      lastAtk: finite(stats.atk),
      baseSpAtk: finite(stats.spAtk),
      lastSpAtk: finite(stats.spAtk),
      baseMove: offense.move || offense.name || 'Ataque',
      lastMove: offense.move || offense.name || 'Ataque',
      expectedTimeline: [{
        at: now,
        kph: Math.max(0, finite(current?.theoreticalKph, current?.kph)),
        xph: Math.max(0, finite(current?.theoreticalXph, current?.xph))
      }],
      meta: {
        ...
        specialItemIds: Array.isArray(current?.specialItemIds) ? current.specialItemIds.map(String) : [],
        npcGoldByItemId: current?.npcGoldByItemId && typeof current.npcGoldByItemId === 'object' ? { ...current.npcGoldByItemId } : {},
  ...
  function expectedStateFromCurrent(current) {
    return {
      kph: Math.max(0, finite(current?.theoreticalKph, current?.kph)),
      xph: Math.max(0, finite(current?.theoreticalXph, current?.xph))
    };
  ...
  function ensureExpectedTimeline(sample) {
    if (!sample) return [];
    if (!Array.isArray(sample.expectedTimeline) || !sample.expectedTimeline.length) {
      sample.expectedTimeline = [{
        at: Math.max(0, finite(sample.startedAt, Date.now())),
        kph: Math.max(0, finite(sample.meta?.expectedKph)),
        xph: Math.max(0, finite(sample.meta?.expectedXph))
      }];
    }
    return sample.expectedTimeline;
  }

  function trackExpectedState(sample, current, now = Date.now()) {
    if (!sample || !current) return false;
    const timeline = ensureExpectedTimeline(sample);
    const next = expectedStateFromCurrent(current);
    const last = timeline[timeline.length - 1] || {};
    if (
      Math.abs(finite(last.kph) - next.kph) < 0.0001
      && Math.abs(finite(last.xph) - next.xph) < 0.01
    ) return false;
    timeline.push({ at: now, kph: next.kph, xph: next.xph });
    if (timeline.length > 250) timeline.splice(0, timeline.length - 250);
    return true;
  }

  function expectedStateAt(sample, at) {
    const timeline = ensureExpectedTimeline(sample);
    let state = {
      kph: Math.max(0, finite(sample?.meta?.expectedKph)),
      xph: Math.max(0, finite(sample?.meta?.expectedXph))
    ...
    for (const point of timeline) {
      if (finite(point?.at) > at) break;
      state = {
        kph: Math.max(0, finite(point?.kph, state.kph)),
        xph: Math.max(0, finite(point?.xph, state.xph))
      };
    }
    return state;
  }

  function expectedAverageForWindow(sample, startAt, endAt) {
    const start = Math.max(0, finite(startAt));
    const end = Math.max(start + 1, finite(endAt));
    const timeline = ensureExpectedTimeline(sample)
      .filter(point => finite(point?.at) <= end)
      .sort((a,b) => finite(a.at) - finite(b.at));

    let cursor = start;
    let state = expectedStateAt(sample, start);
    let kphWeighted = 0;
    let xphWeighted = 0;
    let totalMs = 0;
    ...
    for (const point of timeline) {
      ...
      if (at >= end) break;
      const dt = Math.max(0, at - cursor);
      if (dt > 0) {
        kphWeighted += state.kph * dt;
        xphWeighted += state.xph * dt;
        totalMs += dt;
      }
      state = {
        kph: Math.max(0, finite(point?.kph, state.kph)),
    ...
    if (tail > 0) {
      kphWeighted += state.kph * tail;
      xphWeighted += state.xph * tail;
      totalMs += tail;
    }

    return {
      kph: totalMs ? kphWeighted / totalMs : Math.max(0, finite(sample?.meta?.expectedKph)),
      xph: totalMs ? xphWeighted / totalMs : Math.max(0, finite(sample?.meta?.expectedXph)),
      trackedMs: totalMs
    };
  }

  function advanceExpectedTimeline(sample, boundary) {
    if (!sample) return;
    const state = expectedStateAt(sample, boundary);
    const future = ensureExpectedTimeline(sample)
      .filter(point => finite(point?.at) > boundary)
      .map(point => ({...point}));
    sample.expectedTimeline = [{ at: boundary, kph: state.kph, xph: state.xph }, ...future];
    sample.meta.expectedKph = state.kph;
    sample.meta.expectedXph = state.xph;
  }
  ...
  function createCompletedWindowRow(
    ...
    const loot = lootMeasured
      ? dropDelta(startDrops, endDrops, sample.meta?.rareItemIds, sample.meta?.specialItemIds, sample.meta?.npcGoldByItemId)
      : { items: 0, rareItems: 0, specialItems: 0, npcGold: 0 };
    const items = Math.max(0, finite(loot.items));
    const rareItems = Math.max(0, finite(loot.rareItems));
    const specialItems = Math.max(0, finite(loot.specialItems));
    const npcGold = Math.max(0, finite(loot.npcGold));
    const rawItemsPh = lootMeasured ? items * 2 : null;
    const rawRareItemsPh = lootMeasured ? rareItems * 2 : null;
    const rawSpecialItemsPh = lootMeasured ? specialItems * 2 : null;
    const rawNpcGoldPh = lootMeasured ? npcGold * 2 : null;
    ...
    const cleanBaseSpecialItemsPh = lootMeasured ? rawSpecialItemsPh / dailyLootMultiplier : null;
    const cleanBaseNpcGoldPh = lootMeasured ? rawNpcGoldPh / dailyLootMultiplier : null;

    const levelStart = finite(sample.baseLeadLevel, sample.meta?.leadLevel);
    const levelEnd = finite(sample.lastLeadLevel, levelStart);
    const atkStart = finite(sample.baseAtk);
    const atkEnd = finite(sample.lastAtk, atkStart);
    const spAtkStart = finite(sample.baseSpAtk);
    const spAtkEnd = finite(sample.lastSpAtk, spAtkStart);
    const moveStart = String(sample.baseMove || sample.meta?.move || 'Ataque');
    const moveEnd = String(sample.lastMove || moveStart);
    const levelChanged = levelStart !== levelEnd;
    const moveChanged = compact(moveStart) !== compact(moveEnd);
    const expectedAverage = expectedAverageForWindow(sample, startAt, endAt);
    ...
    return {
      ...
      specialItems,
      specialItemsPh: rawSpecialItemsPh,
      npcGold,
      npcGoldPh: rawNpcGoldPh,
      cleanBaseItemsPh,
      ...
      sampleWindowMinutes: FIXED_MIN_MINUTES,
      ...sample.meta,
      leadLevelStart: levelStart,
      leadLevelEnd: levelEnd,
      atkStart,
      atkEnd,
      spAtkStart,
      spAtkEnd,
      moveStart,
      moveEnd,
      levelingSample: levelChanged || moveChanged,
      move: moveChanged ? `${moveStart} → ${moveEnd}` : moveStart,
      expectedKphAverage: Math.max(0, finite(expectedAverage.kph)),
      expectedXphAverage: Math.max(0, finite(expectedAverage.xph)),
      expectedTrackedSeconds: Math.max(0, finite(expectedAverage.trackedMs)) / 1000,
      calibrationValid: kills >= config.minKills && finite(expectedAverage.kph) > 0
    };
  }
  ...
  function collectCompletedWindows(sample, endAt, reason = 'muestra automática de 30 min') {
    ...
    while (safeEnd - sample.startedAt >= SAMPLE_WINDOW_MS) {
      ...
      if (historySampleValid(row)) segments.push(row);
      completed.push(row);

      sample.startedAt = boundary;
      sample.baseKills = boundaryKills;
      sample.baseXp = boundaryXp;
      sample.baseLeadLevel = finite(sample.lastLeadLevel, sample.baseLeadLevel);
      sample.baseAtk = finite(sample.lastAtk, sample.baseAtk);
      sample.baseSpAtk = finite(sample.lastSpAtk, sample.baseSpAtk);
      sample.baseMove = sample.lastMove || sample.baseMove;
      advanceExpectedTimeline(sample, boundary);
      ...
      if (lootMeasured) {
        ...
        // los drops de los minutos anteriores a 1.1.26. Desde aquí empieza una
  ...
  function updateActiveSample(result, current) {
    ...
    if (!activeSample) beginSample(raw,result,current,now);
      ...
      finalizeActiveSample('cambio de hunt, Pokémon, MT, VIP o bonus', activeSampleSafeEnd(now)); beginSample(raw,result,current,now);
    }
    ...
    const liveLead = result?.lead || {};
    const liveStats = current?.diff?.playerStats || {};
    const liveOffense = current?.diff?.offense || {};
    activeSample.lastLeadLevel = finite(liveLead.level, activeSample.lastLeadLevel);
    activeSample.lastAtk = finite(liveStats.atk, activeSample.lastAtk);
    activeSample.lastSpAtk = finite(liveStats.spAtk, activeSample.lastSpAtk);
    activeSample.lastMove = liveOffense.move || liveOffense.name || activeSample.lastMove || activeSample.baseMove;
    trackExpectedState(activeSample, current, now);

    if (!Array.isArray(activeSample.meta?.rareItemIds)) {
      activeSample.meta.rareItemIds = Array.isArray(current?.rareItemIds) ? current.rareItemIds.map(String) : [];
    }
    if (!Array.isArray(activeSample.meta?.specialItemIds)) {
      activeSample.meta.specialItemIds = Array.isArray(current?.specialItemIds) ? current.specialItemIds.map(String) : [];
    }
    if (!activeSample.meta || typeof activeSample.meta !== 'object') activeSample.meta = {};
    activeSample.meta.npcGoldByItemId = {
      ...(activeSample.meta.npcGoldByItemId && typeof activeSample.meta.npcGoldByItemId === 'object' ? activeSample.meta.npcGoldByItemId : {}),
      ...(current?.npcGoldByItemId && typeof current.npcGoldByItemId === 'object' ? current.npcGoldByItemId : {})
    };
    restoredActiveSample = false;
    ...
    const currentLoot = currentLootMeasured
      ? dropDelta(activeSample.baseDrops, activeSample.lastDrops, activeSample.meta?.rareItemIds, activeSample.meta?.specialItemIds, activeSample.meta?.npcGoldByItemId)
      : {items:0,rareItems:0,specialItems:0,npcGold:0};
    persistActiveSample();
    return {
      ...
      specialItems:currentLootMeasured?currentLoot.specialItems:null,
      npcGold:currentLootMeasured?currentLoot.npcGold:null,
      ...
      specialItemsPh:currentLootMeasured&&hours?currentLoot.specialItems/hours:null,
      npcGoldPh:currentLootMeasured&&hours?currentLoot.npcGold/hours:null,
  ...
  function personalIdentityKeyFromRow(row) {
    const leadKey = compact(row?.leadId || row?.leadName || 'unknown');
    const huntKey = compact(row?.huntKey || row?.slug || row?.huntName || 'unknown');
    return `${leadKey}|${huntKey}`;
  }

  function personalIdentityKeyFromCurrent(lead, hunt) {
    const leadKey = compact(
      lead?.speciesId
      || lead?.pokeId
      || lead?.species?.id
      || lead?.name
  ...
  function rowReferenceLevel(row) {
    return Math.max(1, finite(row?.leadLevelEnd, row?.leadLevel, row?.leadLevelStart, 1));
  }
  ...
  function getPersonalEstimate({
    ...
    const identityKey = personalIdentityKeyFromCurrent(lead, hunt);
    const level = Math.max(1, finite(lead?.level));
    const acct = accountId();

    const rows = segments
      .filter(row =>
        row.accountId === acct
        && historySampleValid(row)
        && personalIdentityKeyFromRow(row) === identityKey
      )
      .sort((a, b) => finite(b.end, b.start) - finite(a.end, a.start))
      .slice(0, HISTORY_WINDOW_SAMPLES);

    if (!rows.length || !(finite(expectedKph) > 0)) return null;

    const speedRows = rows.filter(row => {
      const expected = rowExpectedKphForCalibration(row);
      return expected > 0 && finite(row.kph) > 0;
    });
    if (!speedRows.length) return null;

    const speedFactors = speedRows.map(row =>
      clamp(finite(row.kph) / rowExpectedKphForCalibration(row), 0.60, 1.60)
    );
    const speedStats = trimmedFactor(speedFactors, 1);
    const factor = clamp(speedStats.value, 0.60, 1.60);
    const kph = Math.max(0, finite(expectedKph) * factor);

    const currentMultiplier = (vipActive ? VIP_MULT : 1) * (dailyBoosted ? DAILY_MULT : 1);
    const xpRows = rows.filter(row => {
      const expectedRow = rowExpectedXphForCalibration(row);
      const rowMultiplier = (row.vipActive ? VIP_MULT : 1) * (row.dailyBoosted ? DAILY_MULT : 1);
      const expectedBaseXph = expectedRow > 0 ? expectedRow / rowMultiplier : 0;
      return row.vipKnown === true
        && Number.isFinite(Number(row.cleanBaseXph))
        && finite(row.cleanBaseXph) > 0
        && expectedBaseXph > 0;
    });

    const xpFactors = xpRows.map(row => {
      const rowMultiplier = (row.vipActive ? VIP_MULT : 1) * (row.dailyBoosted ? DAILY_MULT : 1);
      const expectedBaseXph = rowExpectedXphForCalibration(row) / rowMultiplier;
      return clamp(finite(row.cleanBaseXph) / expectedBaseXph, 0.60, 1.60);
    });
    const xpStats = trimmedFactor(xpFactors, factor);
    const xpFactor = clamp(xpStats.value, 0.60, 1.60);

    let baseXph = 0;
    if (finite(expectedXph) > 0) {
      baseXph = Math.max(0, finite(expectedXph) / currentMultiplier * xpFactor);
    } else {
      const knownXpRows = rows.filter(row =>
        row.vipKnown === true
        && Number.isFinite(Number(row.cleanBaseXph))
        && finite(row.cleanBaseXph) > 0
      );
      const avgHistoricalBaseXph = knownXpRows.length
        ? knownXpRows.reduce((sum, row) => sum + finite(row.cleanBaseXph), 0) / knownXpRows.length
        : 0;
      const avgHistoricalKph = speedRows.reduce((sum, row) => sum + finite(row.kph), 0) / speedRows.length;
      baseXph = avgHistoricalKph > 0
        ? avgHistoricalBaseXph * (kph / avgHistoricalKph)
        : avgHistoricalBaseXph;
    }

    const historicalKph = speedRows.reduce((sum, row) => sum + finite(row.kph), 0) / speedRows.length;
    ...
    if (totalKills <= 0 || !(kph > 0) || !(baseXph > 0)) return null;
    ...
    const specialRows = rows.filter(row =>
      row.lootMeasured === true
      && Number.isFinite(Number(row.cleanBaseSpecialItemsPh))
    );
    const goldRows = rows.filter(row =>
      row.lootMeasured === true
      && Number.isFinite(Number(row.cleanBaseNpcGoldPh))
    );

    let baseItemsPh = lootRows.length
      ? lootRows.reduce((sum, row) => sum + Number(row.cleanBaseItemsPh), 0) / lootRows.length
      : null;
    let baseRareItemsPh = rareRows.length
      ? rareRows.reduce((sum, row) => sum + Number(row.cleanBaseRareItemsPh), 0) / rareRows.length
      : null;
    let baseSpecialItemsPh = specialRows.length
      ? specialRows.reduce((sum, row) => sum + Number(row.cleanBaseSpecialItemsPh), 0) / specialRows.length
      : null;
    let baseNpcGoldPh = goldRows.length
      ? goldRows.reduce((sum, row) => sum + Number(row.cleanBaseNpcGoldPh), 0) / goldRows.length
      : null;

    if (historicalKph > 0 && kph > 0) {
      const kphScale = kph / historicalKph;
      if (baseItemsPh !== null) baseItemsPh *= kphScale;
      if (baseRareItemsPh !== null) baseRareItemsPh *= kphScale;
      if (baseSpecialItemsPh !== null) baseSpecialItemsPh *= kphScale;
      if (baseNpcGoldPh !== null) baseNpcGoldPh *= kphScale;
    }

    const levels = rows.map(rowReferenceLevel).filter(value => value > 0);
    const currentLootMultiplier = dailyBoosted ? DAILY_MULT : 1;

    return {
      source: 'historico-reciente',
      matchType: 'ventana-reciente',
      matchLevel: levels.length ? Math.max(...levels) : level,
      matchMinLevel: levels.length ? Math.min(...levels) : level,
      matchMaxLevel: levels.length ? Math.max(...levels) : level,
      currentLevel: level,
      factor,
      speedFactor: factor,
      xpFactor,
      speedFactorSamples: speedStats.total,
      speedFactorSamplesUsed: speedStats.used,
      xpFactorSamples: xpStats.total,
      xpFactorSamplesUsed: xpStats.used,
      outliersDiscarded: Math.max(speedStats.discarded, xpStats.discarded),
      historicalKph,
      baseXph,
      currentXph: baseXph * currentMultiplier,
      kph,
      samples: rows.length,
      ...
      specialSamples: specialRows.length,
      goldSamples: goldRows.length,
      baseItemsPh,
      ...
      baseSpecialItemsPh,
      currentSpecialItemsPh: baseSpecialItemsPh === null ? null : baseSpecialItemsPh * currentLootMultiplier,
      baseNpcGoldPh,
      currentNpcGoldPh: baseNpcGoldPh === null ? null : baseNpcGoldPh * currentLootMultiplier
    };
  }
  ...
  function aggregateHistory() {
    ...
    for (const row of currentPokemonHistoryRows()) {
      ...
      const agg=map.get(key)||{
        key,huntName:row.huntName,leadName:row.leadName,moves:[],
        samples:0,seconds:0,kills:0,cleanXp:0,rawXp:0,rawXpSamples:0,legacy:0,vipYes:0,vipNo:0,
        lootSamples:0,cleanItemsPh:0,rareSamples:0,cleanRareItemsPh:0,specialSamples:0,cleanSpecialItemsPh:0,goldSamples:0,cleanNpcGoldPh:0,
        minLevel:Infinity,maxLevel:0,levelingSamples:0
      };
      agg.samples++; agg.seconds+=SAMPLE_WINDOW_SECONDS; agg.kills+=finite(row.kills);
      const moveLabel = `${String(row.move || 'Ataque')}${row.tm ? ' (MT)' : ''}`;
      if (!agg.moves.includes(moveLabel)) agg.moves.push(moveLabel);
      if(Number.isFinite(Number(row.rawXph))){
        agg.rawXp+=Number(row.rawXph);
        agg.rawXpSamples++;
      }
      const startLevel=finite(row.leadLevelStart,row.leadLevel);
      const endLevel=finite(row.leadLevelEnd,row.leadLevel);
      if(startLevel>0)agg.minLevel=Math.min(agg.minLevel,startLevel);
      if(endLevel>0)agg.maxLevel=Math.max(agg.maxLevel,endLevel);
      if(row.levelingSample===true||startLevel!==endLevel)agg.levelingSamples++;
      ...
      if (row.lootMeasured===true&&Number.isFinite(Number(row.cleanBaseSpecialItemsPh))) {
        agg.specialSamples++; agg.cleanSpecialItemsPh+=Number(row.cleanBaseSpecialItemsPh);
      }
      if (row.lootMeasured===true&&Number.isFinite(Number(row.cleanBaseNpcGoldPh))) {
        agg.goldSamples++; agg.cleanNpcGoldPh+=Number(row.cleanBaseNpcGoldPh);
      }
      map.set(key,agg);
    }
    return [...map.values()].map(row=>{
      ...
      const personalXph=row.rawXpSamples?row.rawXp/row.rawXpSamples:0;
      ...
      const baseSpecialItemsPh=row.specialSamples?row.cleanSpecialItemsPh/row.specialSamples:null;
      const baseNpcGoldPh=row.goldSamples?row.cleanNpcGoldPh/row.goldSamples:null;
      const moveSummary = row.moves.length <= 2
        ? row.moves.join(' / ')
        : `${row.moves.slice(0, 2).join(' / ')} +${row.moves.length - 2}`;
      return {
        ...row,hours,kph:row.samples?row.kills/hours:0,baseXph,personalXph,moveSummary,
        vipXph:baseXph*VIP_MULT,vipDailyXph:baseXph*VIP_MULT*DAILY_MULT,
        baseItemsPh,baseRareItemsPh,baseSpecialItemsPh,baseNpcGoldPh
      };
    }).sort((a,b)=>b.baseXph-a.baseXph||b.samples-a.samples);
  }

  function historyGroupKey(row) {
    return `${compact(row.huntKey||row.slug||row.huntName)}|${compact(row.leadId||row.leadName)}`;
  }
  ...
  function clearHistory(){
    segments=[];calibrationRegistry={};activeSample=null;
    try{
      localStorage.setItem(HISTORY_CLEARED_KEY,'1');
  ...
  function renderCurrentHtml() {
    ...
    const observedSpecialPh=Number.isFinite(Number(evaluation?.specialItemsPh))?Number(evaluation.specialItemsPh):(session.lootMeasured&&Number.isFinite(Number(session.specialItemsPh))?Number(session.specialItemsPh):null);
    const observedNpcGoldPh=Number.isFinite(Number(evaluation?.npcGoldPh))?Number(evaluation.npcGoldPh):(session.lootMeasured&&Number.isFinite(Number(session.npcGoldPh))?Number(session.npcGoldPh):null);
  ...
  function renderHistoryHtml() {
    ...
    const totalStored=segments.filter(row=>row.accountId===accountId()&&finite(row.elapsedSeconds)===SAMPLE_WINDOW_SECONDS).length;
    return `<div class="pg-u-note"><b>Histórico de ${esc(pokemon.name)}</b>${pokemon.level?` Nv. ${fmt(pokemon.level)}`:''}. Muestras totales guardadas en esta cuenta: <b>${fmt(totalStored)}</b>. Solo se muestran abajo las del Pokémon activo.<br>Cada Hunt + Pokémon conserva una ventana móvil de <b>${HISTORY_WINDOW_SAMPLES} muestras válidas</b> de 30 minutos (≈ ${fmt(HISTORY_WINDOW_SAMPLES * 0.5, 1)} h). Al entrar una nueva cuando la ventana está completa, se elimina automáticamente la más antigua. El nivel y los cambios de stats no invalidan el histórico: cada tanda se compara con lo que PIWTools esperaba durante esa propia tanda. Con al menos ${HISTORY_TRIM_MIN_SAMPLES} factores se descartan el mejor y el peor antes de calcular la calibración reciente. <b>XP/h real histórico</b> es experiencia realmente observada; <b>EXP base</b> elimina VIP ×1,50 y bonus diario ×1,20. Items/h, Raros/h, Especiales/h y Oro NPC/h usan drops realmente observados cuando existe muestra. Especiales/h suma Strange Pheromone + Bronze Boss Token a valor 1:1; Oro NPC/h excluye siempre ambos especiales para evitar que su alto valor NPC distorsione el histórico.</div>${legacy?`<div class="pg-hi-banner warn">Hay ${fmt(legacy)} muestras antiguas de ${esc(pokemon.name)} sin estado VIP. Sus kills/h siguen siendo útiles, pero su EXP no se usa hasta clasificarlas. <button data-adopt-legacy-vip>Asignarles VIP actual: ${vip?'Sí':'No'}</button></div>`:''}
      <div class="pg-hi-history-head"><span>Hunt · Pokémon</span><span>Muestras</span><span>Horas</span><span>Kills/h</span><span>XP/h real histórico</span><span>EXP base</span><span>Items/h base</span><span>Raros/h base</span><span>Especiales/h</span><span></span></div>
      <div class="pg-hi-history">${rows.map(row=>`<div class="pg-hi-history-row"><span class="pg-hi-history-name"><b>${esc(row.huntName||'Hunt')}</b><small>${esc(row.leadName||pokemon.name||'Pokémon')} · ${row.minLevel!==Infinity&&row.maxLevel?`Nv. ${fmt(row.minLevel)}${row.maxLevel!==row.minLevel?`→${fmt(row.maxLevel)}`:''} · `:''}${esc(row.moveSummary||'Ataque')}${row.levelingSamples?` · leveo ${row.levelingSamples}`:''}${row.legacy?` · ${row.legacy} legado`:''}${row.lootSamples?` · loot ${row.lootSamples}/${row.samples}`:' · loot aún sin muestra'}</small></span><span>${fmt(row.samples)}/${HISTORY_WINDOW_SAMPLES}</span><span>${fmt(row.hours,1)}</span><span>${fmt(row.kph)}</span><span>${row.personalXph?fmt(row.personalXph):'—'}</span><span>${row.baseXph?fmt(row.baseXph):'—'}</span><span>${row.baseItemsPh===null?'—':fmt(row.baseItemsPh,2)}</span><span>${row.baseRareItemsPh===null?'—':fmt(row.baseRareItemsPh,2)}</span><span>${row.baseSpecialItemsPh===null?'—':fmt(row.baseSpecialItemsPh,2)}</span><button class="pg-hi-delete" data-delete-intelligence-history="${esc(row.key)}" data-history-label="${esc(`${row.huntName||'Hunt'} · ${row.leadName||pokemon.name||'Pokémon'}`)}" title="Borrar todas las muestras de esta línea" aria-label="Borrar histórico de ${esc(row.huntName||'esta hunt')}">🗑️</button></div>`).join('')||`<div class="pg-u-empty">${esc(pokemon.name)} todavía no tiene muestras completas. La primera aparecerá al completar 30 minutos útiles en la misma hunt y condiciones.</div>`}</div>
  ...
  function combatStateSignature() {
    const lead = liveLeadForWatcher();
    if (!lead) return '';
    const species = compact(lead.speciesId || lead.pokeId || lead.species?.id || lead.name || '');
    const level = finite(lead.level);
    const atk = finite(
      lead?.stats?.atk,
      lead?.stats?.attack,
      lead?.atk,
    ...
    const spa = finite(
      lead?.stats?.spAtk,
      lead?.stats?.spa,
      lead?.stats?.specialAttack,
      lead?.spAtk,
      lead?.spa,
      lead?.specialAttack,
      lead?.attributes?.spa
    );
    return [species, level, atk, spa].join('|');
  }
  ...
  function checkHistoryPokemonChange(){
    ...
    const nextCombatSignature=combatStateSignature();
    const pokemonChanged=Boolean(lastHistoryPokemonKey&&nextKey!==lastHistoryPokemonKey);
    const combatChanged=Boolean(
      lastCombatStateSignature
      && nextCombatSignature
      && nextCombatSignature!==lastCombatStateSignature
    );

    lastHistoryPokemonKey=nextKey;
    lastCombatStateSignature=nextCombatSignature;

    if(!pokemonChanged&&!combatChanged)return false;

    const reason=pokemonChanged?'pokemon-changed':'pokemon-stats-changed';
    try{
      window.dispatchEvent(new CustomEvent('pokegrid-intelligence-updated',{
        detail:{reason,pokemonKey:nextKey,combatSignature:nextCombatSignature}
      }));
    }catch{}
    requestContextRefresh();
    return true;
  ...
  function startHistoryPokemonWatcher(){
    ...
    lastCombatStateSignature=combatStateSignature();
  ...
  window.__PGHuntIntelligenceSupervisor = {
    version:'1.1.46',refresh,getState:state,getReport:()=>clone(lastReport),getHistory:()=>clone(segments),getCurrentHistoryPokemon:()=>clone(currentHistoryPokemon()),getPersonalEstimate,getCalibration,
  ...
  window.__PGPerformanceSupervisor = Object.freeze({ version:'1.1.46',getState:state,refresh:()=>refresh(true),getHistory:()=>clone(segments),clearHistoryEntry,clearHistory });

  let healthClient=null;
  function connectHealth(){const bridge=window.__pokeGridScripts;if(!bridge?.register||healthClient)return Boolean(healthClient);healthClient=bridge.register({id:'performance-supervisor',name:'Supervisor de rendimiento Hunt Intelligence',version:'1.1.46',description:'Mide rendimiento real con ventana móvil de 12 muestras por Hunt + Pokémon, incluidos Items/h, Raros/h, Especiales/h y Oro NPC/h real sin especiales, y normaliza XP/VIP/bonus diario.',icon:'📈',category:'gameplay-analysis',status:'waiting',statusText:'Esperando una muestra.',staleAfterMs:50000,capabilities:['real-kph','real-items-ph','real-rare-items-ph','real-special-items-ph','real-npc-gold-ph','piwtools-comparison','history','segmentation','vip-normalization','daily-normalization','loot-daily-normalization','personal-ranking']});healthClient.registerCommand('open',()=>{try{window.__PGHuntIntelligence?.openPerformance?.();}catch{}return{opened:true};},{label:'Abrir rendimiento'});healthClient.registerCommand('refresh',()=>refresh(true),{label:'Actualizar medición'});healthClient.registerCommand('get-history',()=>clone(segments),{label:'Obtener histórico'});healthClient.registerCommand('clear-history',clearHistory,{label:'Borrar histórico',dangerous:true});setInterval(()=>{try{healthClient.heartbeat(state());}catch{}},10000);try{healthClient.heartbeat(state());}catch{}return true;}
  ...
  const invalidatedLootRows = migrateBrokenLootRows();
  restartTimer();
  restartSampleCheckpoint();
  startHistoryPokemonWatcher();
  setTimeout(()=>refresh(false),1200);
  console.info(`[Hunt Intelligence] Supervisor unificado v1.1.39 cargado: ventana móvil ${HISTORY_WINDOW_SAMPLES} por Hunt + Pokémon; recuperación ${historyRecovery.source}:${historyRecovery.recovered}; ${invalidatedLootRows} muestra(s) 0/0 invalidadas.`);
})();

(() => {
  'use strict';
  if (window.__pgHuntIntelligenceUiV1138) return;
  window.__pgHuntIntelligenceUiV1138 = true;
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .pg-hi-history-head,#${PANEL_ID} .pg-hi-history-row{display:grid;grid-template-columns:minmax(155px,1.45fr) 58px 58px 76px 100px 92px 88px 82px 88px 40px;gap:6px;align-items:center;padding:8px 9px;font-size:10px}.pg-hi-history-head{color:#8392a3;text-transform:uppercase}.pg-hi-history{display:grid;gap:7px}.pg-hi-history-row{background:#111923;border:1px solid #273443;border-radius:8px}.pg-hi-history-row small{display:block;color:#8795a6;margin-top:2px}.pg-hi-delete{width:34px;height:32px;padding:0!important;color:#ffaaa3!important;border-color:#6d3b3b!important;background:#291718!important}.pg-hi-delete:hover{background:#4a2020!important;border-color:#a85656!important}.pg-hi-actions{display:flex;justify-content:flex-end;margin-top:10px}
      ...
      #${PANEL_ID} .pg-u-table-head,#${PANEL_ID} .pg-u-row{display:grid;grid-template-columns:34px minmax(175px,1fr) repeat(7,minmax(70px,auto));gap:8px;align-items:center}
      ...
      #${PANEL_ID} .pg-u-metric{text-align:right;font-variant-numeric:tabular-nums}.piw-xp{color:#72b7ff}.real-xp{color:#64d8c0}.xp{color:#72b7ff}.gold{color:#f2cc60}.loot{color:#8ce99a}.rare{color:#ff9fd5}.special{color:#ffcf70}.score{color:#d4a6ff}.rate{color:#d5a6ff}.speed{color:#72b7ff}.items{color:#8ce99a}.eff{color:#ffd36c}
  ...
  function createBridgeUiWindow() {
    ...
    if (uiWindow?.panel?.isConnected && uiWindow.panel.id === PANEL_ID) {
      usingBridgeUi = true;
      return uiWindow;
    }
  ...
  function shell(bodyHtml, options = {}) {
    ...
    let overlay = null;
    const hadManagedWindow = Boolean(uiWindow?.panel?.isConnected && uiWindow.panel.id === PANEL_ID);
    ...
    if (managed) {
      ...
      if (previous) {
        const previousCard = previous.querySelector('.pg-u-card');
        if (previousCard) {
          savePanelLayout(previousCard);
          try { previousCard.__pgResizeObserver?.disconnect?.(); } catch {}
        }
        previous.remove();
      }
    ...
    if (overlay.__pgHuntClickHandler) {
      overlay.removeEventListener('click', overlay.__pgHuntClickHandler);
    }
    const clickHandler = event => {
      ...
      if (weightButton) {
        ...
        if (['xpWeight', 'lootWeight', 'rareWeight', 'specialWeight', 'goldWeight'].includes(key)) {
  ...
  function renderHunt(result) {
    ...
    const body = `
      ...
      ${cfg.mode === 'general' && finite(cfg.specialWeight, 0) > 0 && !finite(result.specialKnownRowsCount, 0) ? `<div class="pg-u-note"><b>Especiales/h:</b> todavía no hay rate visible ni muestras nuevas de 30 min con datos de Strange Pheromone/Bronze Tokens para estas hunts. El peso está guardado, pero empezará a discriminar objetivos cuando exista información medible.</div>` : ''}
      <div class="pg-u-settings" ${cfg.mode === 'general' ? '' : 'style="display:none"'}>
        ...
        ${weightControlHtml('specialWeight', 'Peso especiales/h', cfg.specialWeight)}
      ...
      <div class="pg-u-table-head">
        ...
        <div class="pg-u-col">Tu XP/h estimada</div>
        <div class="pg-u-col">Items/h</div>
        <div class="pg-u-col hide-mobile">Raros/h</div>
        <div class="pg-u-col">Especiales/h</div>
      ...
      <div>${topRows.map((row, index) => `
        <div class="pg-u-row ${index === 0 ? 'best' : ''}">
          ...
          <div class="pg-u-metric special"><b>${row.specialDataKnown ? fmt(row.specialPh, 2) : '—'}</b></div>
  ...
  const FAVORITES_OPACITY_MIGRATION_KEY = `${FAVORITES_NS}:opacity-v2`;
  ...
  function favoriteCurrentAccount() {
    const character = H()?.getCharacter?.() || window.__poke?.api?.['/api/characters/me']?.character || {};
  ...
  function favoriteEnsureStyles() {
    ...
    style.textContent = `
      #${FAVORITES_PANEL_ID}.pg-ui-window{
        max-width:520px!important;
        background:rgba(12,19,29,var(--pg-ui-opacity,.92))!important;
        border-color:rgba(84,105,132,.88)!important;
        box-shadow:0 16px 46px rgba(0,0,0,.68)!important;
        backdrop-filter:blur(4px)!important;
        -webkit-backdrop-filter:blur(4px)!important;
      }
      #${FAVORITES_PANEL_ID} .pg-ui-header{padding:7px 8px;}
      #${FAVORITES_PANEL_ID} .pg-ui-opacity{display:flex!important;min-width:94px!important;gap:4px!important;font-size:8px!important;}
      #${FAVORITES_PANEL_ID} .pg-ui-opacity input[type="range"]{width:52px!important;}
      #${FAVORITES_PANEL_ID} .pg-ui-opacity-value{width:28px!important;}
  ...
  function favoriteEnsureWindow() {
    ...
    if (!favoritesUiWindow?.panel?.isConnected) {
      ...
      // 1.1.38: la 1.1.38 ocultaba el medidor y podía heredar una opacidad
      // prácticamente nula del UI Core. Se migra una sola vez a un valor visible.
      try {
        const migrated = localStorage.getItem(FAVORITES_OPACITY_MIGRATION_KEY) === '1';
        const currentOpacity = Number(favoritesUiWindow?.getOpacity?.());
        if (!migrated || !Number.isFinite(currentOpacity) || currentOpacity < 55) {
          favoritesUiWindow?.setOpacity?.(92);
          localStorage.setItem(FAVORITES_OPACITY_MIGRATION_KEY, '1');
        }
      } catch {}
    }
  ...
  async function favoriteExecuteCurrentAccount(reason = 'command') {
    ...
    favoritesExecution = (async () => {
      ...
      try {
        ...
        const lead = H()?.getLeadPokemon?.();
        if (!lead) {
          favoriteUpdateAccountStatus(account.id, 'Esperando Pokémon activo');
          return false;
        }
        if (!H()?.isUnlocked?.(hunt, lead)) {
          favoriteUpdateAccountStatus(account.id, `Bloqueada: requiere Nv.${fmt(H()?.huntRequiredLevel?.(hunt) || 1)}`, { running: false });
          return false;
        }
  ...
  function favoriteWatchdog() {
    ...
    Promise.resolve(H()?.getData?.(false)).then(data => {
  ...
  function favoriteInstallMessageBridge() {
    ...
    const runRpcAction = async (action, args) => {
      ...
      switch (String(action || '')) {
        case 'ping':
          return { ready: true, version: '1.1.46', account: favoriteGetLocalAccountState() };
  ...
  function favoriteInitializeSafely() {
    try {
      favoriteConnectChannel();
      favoriteInstallMessageBridge();
      favoriteRegisterCurrentAccount();
      ...
      console.error('[Hunt Intelligence · Favoritos] El módulo no pudo inicializarse; Hunt Intelligence principal seguirá disponible.', error);
      return false;
    }
  }
  favoriteInitializeSafely();


  // ===== API local por webview para el panel global de PokeGrid =====
  function favoriteGetLocalAccountState() {
    const account = favoriteRegisterCurrentAccount();
    if (!account) return null;
    const store = favoriteReadStore();
    const row = store.accounts[account.id]
      ? favoriteNormalizeEntry(account.id, store.accounts[account.id])
      : favoriteNormalizeEntry(account.id, { name: account.name });
    return {
      ...row,
      current: true,
      online: true
  ...
  function favoriteSetLocalEnabled(enabled) {
    const account = favoriteRegisterCurrentAccount();
    if (!account) return { ok: false, error: 'Cuenta actual no disponible.' };
    const ok = favoriteSetEnabled(account.id, Boolean(enabled));
    return { ok, account: favoriteGetLocalAccountState() };
  }

  async function favoriteSetLocalTarget(slot, slug) {
    const account = favoriteRegisterCurrentAccount();
    if (!account) return { ok: false, error: 'Cuenta actual no disponible.' };
    await favoriteLoadChoices();
    const ok = favoriteSetTargetSlot(account.id, Number(slot), String(slug || ''));
    return { ok, account: favoriteGetLocalAccountState() };
  }
  ...
  async function favoriteStartLocalAccount() {
    const account = favoriteRegisterCurrentAccount();
    if (!account) return { ok: false, started: false, error: 'Cuenta actual no disponible.' };

    const store = favoriteReadStore();
    const row = favoriteNormalizeEntry(account.id, store.accounts[account.id]);
    if (!row.enabled) {
      row.running = false;
      row.lastStatus = 'Desactivada';
      row.lastStatusAt = Date.now();
      store.accounts[account.id] = row;
      favoriteWriteStore(store);
      return { ok: true, started: false, reason: 'disabled', account: favoriteGetLocalAccountState() };
    }
    if (!row.favorites[0]) {
      row.running = false;
      row.lastStatus = 'Sin objetivo #1';
      row.lastStatusAt = Date.now();
      store.accounts[account.id] = row;
      favoriteWriteStore(store);
      return { ok: true, started: false, reason: 'no-target', account: favoriteGetLocalAccountState() };
    }
    ...
    const confirmed = await favoriteExecuteCurrentAccount('pokegrid-shell-start');
    return {
      ok: true,
      started: true,
  ...
  function favoriteStopLocalAccount() {
    const account = favoriteRegisterCurrentAccount();
    if (!account) return { ok: false, error: 'Cuenta actual no disponible.' };
    const store = favoriteReadStore();
    const row = favoriteNormalizeEntry(account.id, store.accounts[account.id]);
    ...
    store.accounts[account.id] = row;
    favoriteWriteStore(store, { announce: false });
    return { ok: true, account: favoriteGetLocalAccountState() };
  }
  ...
  window.__PGHuntIntelligence = Object.freeze({
    ...
    getFavoriteLocalAccountState: () => favoriteGetLocalAccountState(),
    ...
    startFavoriteLocalAccount: () => favoriteStartLocalAccount(),
    stopFavoriteLocalAccount: () => favoriteStopLocalAccount(),
  ...
  window.addEventListener('pokegrid-intelligence-updated', event => {
    if (busy) return;
    const overlay = document.getElementById(PANEL_ID);
    if (!overlay?.isConnected) return;

    const reason = event?.detail?.reason || '';
    if (reason === 'pokemon-changed' || reason === 'pokemon-stats-changed') {
      rerunActivePanel(false);
      return;
    }
  ...
  install();
  console.info('[Hunt Intelligence] v1.1.39 cargado: gestor compacto de Favoritos conectado al webview de PokeGrid por postMessage y histórico móvil de 12 muestras.');
})();
